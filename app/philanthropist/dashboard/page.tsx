...
import {
  ...
  Coins, Copy, RefreshCw, ChevronRight, X, Zap, Award
} from 'lucide-react';
...
async function getPhilanthropistRecord(userId: string) {
...
async function ensurePhilanthropistRecord(userId: string) {
  let record = await getPhilanthropistRecord(userId);
  if (!record) {
    const { data } = await supabase
      .from('philanthropists')
...
export default function PhilanthropistDashboardPage() {
  ...
  useEffect(() => {
    ...
    const isPhil = user.role === 'philanthropist' || user.email?.toLowerCase() === 'dinfadashe@gmail.com';
    if (!isPhil) { router.push('/beneficiary-dashboard'); return; }
    loadAll();
  }, [user, authLoading]);
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const record = await ensurePhilanthropistRecord(user.id);
  ...
  const handleActivate = async (item: any) => {
    ...
    try {
      ...
      // Deduct ACT from philanthropist
      const newBalance = actBalance - ACT_PER_ACTIVATION;
      const newTotal = totalActivated + 1;
      await supabase
        .from('philanthropists')
      ...
      showToast(`✓ ${item.users?.full_name || item.users?.email} activated! −${ACT_PER_ACTIVATION} ACT`, 'success');
      await loadAll();
    } catch (e: any) {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#0A1628', color: 'white', fontFamily: 'sans-serif' }}>
      ...
      {/* TOAST */}
      {toast && (
        <div style={{ position: 'fixed', top: 20, right: 20, zIndex: 999, padding: '14px 20px', borderRadius: 14, backgroundColor: toast.type === 'success' ? 'rgba(0,184,148,0.15)' : 'rgba(255,107,107,0.15)', border: `1px solid ${toast.type === 'success' ? 'rgba(0,184,148,0.5)' : 'rgba(255,107,107,0.5)'}`, color: toast.type === 'success' ? '#00B894' : '#ff6b6b', fontWeight: 600, fontSize: 14, display: 'flex', alignItems: 'center', gap: 10, boxShadow: '0 12px 40px rgba(0,0,0,0.4)', maxWidth: 360 }}>
          {toast.type === 'success' ? <CheckCircle style={{ width: 18, height: 18, flexShrink: 0 }} /> : <AlertCircle style={{ width: 18, height: 18, flexShrink: 0 }} />}
          {toast.msg}
        </div>
      )}
      ...
      {showRefill && (
        <RefillModal
          ...
          onSuccess={(newBalance) => { setActBalance(newBalance); setShowRefill(false); showToast('ACT balance refilled successfully! +1000 ACT', 'success'); loadAll(); }}
          userId={user?.id || ''}
          showToast={showToast}
        />
      )}
      ...
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 800, height: 800, background: 'radial-gradient(circle, rgba(0,184,148,0.05) 0%, transparent 70%)', borderRadius: '50%' }} />
      </div>
      ...
      <header style={{ position: 'sticky', top: 0, zIndex: 50, borderBottom: '1px solid rgba(255,255,255,0.08)', backgroundColor: 'rgba(10,22,40,0.95)', backdropFilter: 'blur(12px)' }}>
        <div style={{ maxWidth: 860, margin: '0 auto', padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontSize: 11, color: '#00B894', backgroundColor: 'rgba(0,184,148,0.15)', padding: '3px 10px', borderRadius: 999, border: '1px solid rgba(0,184,148,0.3)', fontWeight: 700, letterSpacing: 0.5 }}>PHILANTHROPIST</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <main style={{ maxWidth: 860, margin: '0 auto', padding: '28px 20px 80px', position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: 'clamp(24px, 4vw, 32px)', fontWeight: 800, marginBottom: 6 }}>
        ...
        {/* ACT BALANCE CARD */}
        <div style={{ marginBottom: 20, padding: '24px', borderRadius: 20, background: 'linear-gradient(135deg, #0d2137 0%, #0a1628 60%, #0d2137 100%)', border: `2px solid ${actColor}40`, boxShadow: `0 8px 40px ${actColor}15`, position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: -30, right: -30, width: 160, height: 160, borderRadius: '50%', background: `${actColor}08` }} />
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                <p style={{ fontSize: 48, fontWeight: 900, color: actColor, lineHeight: 1, margin: 0 }}>{actBalance.toLocaleString()}</p>
                <span style={{ fontSize: 16, color: '#8FA3BF', fontWeight: 600 }}>ACT</span>
              </div>
              <p style={{ fontSize: 12, color: '#8FA3BF', marginTop: 4 }}>
                {Math.floor(actBalance / ACT_PER_ACTIVATION)} activations remaining · {ACT_PER_ACTIVATION} ACT per activation
              </p>
            </div>
            <button
              onClick={() => setShowRefill(true)}
              style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 18px', borderRadius: 12, background: `linear-gradient(to right, ${actColor}, #00CEC9)`, color: 'white', fontWeight: 700, fontSize: 13, border: 'none', cursor: 'pointer', boxShadow: `0 4px 16px ${actColor}30`, flexShrink: 0 }}
            >
          ...
          {/* Progress bar */}
          <div style={{ backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 999, height: 8, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${actPct}%`, background: `linear-gradient(to right, ${actColor}, #00CEC9)`, borderRadius: 999, transition: 'width 0.5s ease' }} />
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
            <span style={{ fontSize: 11, color: '#8FA3BF' }}>0</span>
            <span style={{ fontSize: 11, color: '#8FA3BF' }}>{ACT_INITIAL_BALANCE} ACT max</span>
          </div>
          ...
          {actBalance < 100 && (
            <div style={{ marginTop: 14, padding: '10px 14px', borderRadius: 10, backgroundColor: 'rgba(255,107,107,0.1)', border: '1px solid rgba(255,107,107,0.25)', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <p style={{ fontSize: 12, color: '#ff6b6b', margin: 0 }}>
                Low balance! You can only activate {Math.floor(actBalance / ACT_PER_ACTIVATION)} more beneficiar{Math.floor(actBalance / ACT_PER_ACTIVATION) === 1 ? 'y' : 'ies'}. <button onClick={() => setShowRefill(true)} style={{ background: 'none', border: 'none', color: '#ff6b6b', textDecoration: 'underline', cursor: 'pointer', fontWeight: 700, padding: 0, fontSize: 12 }}>Refill now →</button>
              </p>
            </div>
          )}
        </div>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: '#8FA3BF', lineHeight: 1.7, margin: 0 }}>
                ...
                When your balance runs out, refill with <strong style={{ color: 'white' }}>${REFILL_COST_USD} USDT</strong> to receive <strong style={{ color: '#00B894' }}>{REFILL_ACT_AMOUNT} ACT</strong>.
                <strong style={{ color: '#ff6b6b' }}> Overcharging beneficiaries will result in immediate removal.</strong>
              </p>
            </div>
          </div>
        </div>
        ...
        {/* ACTIVATE BY EMAIL */}
        <ActivateByEmail onActivated={loadAll} showToast={showToast} actBalance={actBalance} onDeductAct={(newBal, newTotal) => { setActBalance(newBal); setTotalActivated(newTotal); }} userId={user?.id || ''} onLowBalance={() => setShowRefill(true)} />
...
// ── Activate by email component ───────────────────────────────────────────────

function ActivateByEmail({ onActivated, showToast, actBalance, onDeductAct, userId, onLowBalance }: {
  onActivated: () => void;
  showToast: (msg: string, type: 'success' | 'error') => void;
  actBalance: number;
  onDeductAct: (newBal: number, newTotal: number) => void;
  userId: string;
  onLowBalance: () => void;
}) {
  ...
  const searchUser = async () => {
    ...
    try {
      const { data: userData } = await supabase.from('users').select('*').ilike('email', email.trim()).single();
      if (!userData) { setNotFound(true); return; }
      const { data: activation } = await supabase.from('beneficiary_activations').select('*').eq('user_id', userData.id).single();
  ...
  const handleActivate = async () => {
    ...
    if (actBalance < ACT_PER_ACTIVATION) { showToast('Insufficient ACT. Please refill.', 'error'); onLowBalance(); return; }
    setActivating(true);
    try {
      if (foundUser.activation?.payment_status === 'verified') { showToast('This account is already activated.', 'error'); return; }
      if (foundUser.activation) {
        const { error } = await supabase.from('beneficiary_activations').update({ payment_status: 'verified', activation_method: 'philanthropist', updated_at: new Date().toISOString() }).eq('user_id', foundUser.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('beneficiary_activations').insert({ user_id: foundUser.id, payment_status: 'verified', activation_method: 'philanthropist', created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
        if (error) throw error;
      }
      // Deduct ACT
      const { data: phil } = await supabase.from('philanthropists').select('act_balance, total_activated').eq('user_id', userId).single();
      const newBal = (phil?.act_balance ?? actBalance) - ACT_PER_ACTIVATION;
      const newTotal = (phil?.total_activated ?? 0) + 1;
      await supabase.from('philanthropists').update({ act_balance: newBal, total_activated: newTotal, updated_at: new Date().toISOString() }).eq('user_id', userId);
      onDeductAct(newBal, newTotal);
      showToast(`✓ ${foundUser.full_name || foundUser.email} activated! −${ACT_PER_ACTIVATION} ACT`, 'success');
    ...
    <div style={{ padding: '22px', borderRadius: 18, border: '1px solid rgba(0,206,201,0.2)', backgroundColor: 'rgba(255,255,255,0.03)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 11, color: '#8FA3BF', margin: 0 }}>Search and activate a specific beneficiary by email</p>
        </div>
      </div>

      <div style={{ display: 'flex', gap: 10, marginBottom: foundUser || notFound ? 14 : 0 }}>
        <input
          placeholder="Enter beneficiary's registered email"
          value={email}
        ...
        />
        <button onClick={searchUser} disabled={searching || !email.trim()} style={{ padding: '12px 20px', borderRadius: 10, background: 'linear-gradient(to right, #00CEC9, #00B894)', color: 'white', fontWeight: 700, fontSize: 13, border: 'none', cursor: (searching || !email.trim()) ? 'not-allowed' : 'pointer', opacity: (searching || !email.trim()) ? 0.6 : 1, whiteSpace: 'nowrap' }}>
      ...
      {notFound && (
        <div style={{ padding: '12px 14px', borderRadius: 10, backgroundColor: 'rgba(255,107,107,0.08)', border: '1px solid rgba(255,107,107,0.2)' }}>
          <p style={{ fontSize: 13, color: '#ff6b6b', margin: 0 }}>No account found with that email address. The user must register first.</p>
        </div>
      )}
      ...
      {foundUser && (
        <div style={{ padding: '16px', borderRadius: 12, backgroundColor: 'rgba(0,206,201,0.04)', border: '1px solid rgba(0,206,201,0.15)', display: 'flex', alignItems: 'center', gap: 14 }}>
          {foundUser.profile_picture ? (
            <img src={foundUser.profile_picture} alt={foundUser.full_name} style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(0,206,201,0.3)', flexShrink: 0 }} />
          ) : (
            <div style={{ width: 48, height: 48, borderRadius: '50%', backgroundColor: 'rgba(0,206,201,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <span style={{ fontSize: 20, fontWeight: 800, color: '#00CEC9' }}>{(foundUser.full_name || foundUser.email || 'U')[0].toUpperCase()}</span>
            </div>
          )}
          <div style={{ flex: 1 }}>
            <p style={{ fontWeight: 700, fontSize: 15, color: 'white', marginBottom: 2 }}>{foundUser.full_name || 'No name'}</p>
          ...
          {foundUser.activation?.payment_status !== 'verified' && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
              <button onClick={handleActivate} disabled={activating || actBalance < ACT_PER_ACTIVATION} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '11px 20px', borderRadius: 10, background: actBalance < ACT_PER_ACTIVATION ? 'rgba(255,107,107,0.15)' : 'linear-gradient(to right, #00B894, #00CEC9)', color: actBalance < ACT_PER_ACTIVATION ? '#ff6b6b' : 'white', fontWeight: 700, fontSize: 13, border: actBalance < ACT_PER_ACTIVATION ? '1px solid rgba(255,107,107,0.3)' : 'none', cursor: (activating || actBalance < ACT_PER_ACTIVATION) ? 'not-allowed' : 'pointer', opacity: activating ? 0.7 : 1 }}>
                {activating ? 'Activating...' : <><CheckCircle style={{ width: 15, height: 15 }} /> Activate</>}
              </button>
...
function RefillModal({ onClose, onSuccess, userId, showToast }: {
  onClose: () => void;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: existing } = await supabase.from('philanthropists').select('id').eq('last_refill_hash', cleanHash).single();
      if (existing) { setError('This transaction has already been used for a refill.'); setLoading(false); return; }
      ...
      if (!receipt) { setError('Transaction not found. Please check the hash and ensure it is confirmed on BSC.'); setLoading(false); return; }
      if (receipt.status !== '0x1') { setError('Transaction failed on blockchain. Please use a successful transaction.'); setLoading(false); return; }

      const TRANSFER_TOPIC = '0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef';
      const USDT_CONTRACT = '0x55d398326f99059ff775485246999027b3197955';
      const YOUR_WALLET = WALLET_ADDRESS.toLowerCase();
      const MIN_AMOUNT = 68; // slightly below $70 for rounding
      ...
      for (const log of receipt.logs || []) {
        if (log.address?.toLowerCase() === USDT_CONTRACT && log.topics?.[0]?.toLowerCase() === TRANSFER_TOPIC && log.topics?.length >= 3) {
          ...
          if (toAddress === YOUR_WALLET) {
            ...
            const amount = Number(rawBig) / 1e18;
            const amount6 = Number(rawBig) / 1e6;
            const finalAmount = amount < 0.000001 ? amount6 : amount;
            if (finalAmount >= MIN_AMOUNT) { verified = true; break; }
            else { setError(`Insufficient amount: ${finalAmount.toFixed(2)} USDT. Required: $${REFILL_COST_USD}.`); setLoading(false); return; }
          }
        }
      }

      if (!verified) { setError(`No USDT transfer of $${REFILL_COST_USD} to our wallet found in this transaction.`); setLoading(false); return; }

      // Refill ACT
      const { data: phil } = await supabase.from('philanthropists').select('act_balance').eq('user_id', userId).single();
      const newBalance = (phil?.act_balance ?? 0) + REFILL_ACT_AMOUNT;
      await supabase.from('philanthropists').update({ act_balance: newBalance, last_refill_hash: cleanHash, last_refill_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('user_id', userId);
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, zIndex: 200, backdropFilter: 'blur(4px)' }}>
      <div style={{ width: '100%', maxWidth: 500, backgroundColor: '#0F1F35', border: '1px solid rgba(0,206,201,0.2)', borderRadius: 22, padding: 28, boxShadow: '0 40px 80px rgba(0,0,0,0.6)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: 'linear-gradient(to right, #00B894, #00CEC9)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 11, color: '#8FA3BF', margin: 0 }}>+{REFILL_ACT_AMOUNT} ACT for ${REFILL_COST_USD} USDT</p>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 20 }}>
          ...
          <div style={{ padding: '14px 16px', borderRadius: 12, backgroundColor: 'rgba(0,206,201,0.04)', border: '1px solid rgba(0,206,201,0.12)' }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: '#00CEC9', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <p style={{ fontSize: 11, color: '#ffc107', marginTop: 8, marginBottom: 0 }}>⚠️ Only send on BNB Smart Chain (BEP20). Other networks = lost funds.</p>
          </div>

          {/* Step 2 */}
          <div style={{ padding: '14px 16px', borderRadius: 12, backgroundColor: 'rgba(0,184,148,0.04)', border: '1px solid rgba(0,184,148,0.12)' }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: '#00B894', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <input
              placeholder="0x... paste your BSC transaction hash"
              value={txHash}
        ...
        <div style={{ padding: '12px 16px', borderRadius: 10, backgroundColor: 'rgba(0,184,148,0.06)', border: '1px solid rgba(0,184,148,0.15)', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <p style={{ fontSize: 12, color: '#8FA3BF', margin: 0 }}>
            You will receive <strong style={{ color: '#00B894' }}>{REFILL_ACT_AMOUNT} ACT</strong> — enough to activate <strong style={{ color: 'white' }}>{REFILL_ACT_AMOUNT / ACT_PER_ACTIVATION} beneficiaries</strong>.
          </p>
        </div>

        <button onClick={handleSubmit} disabled={loading || !txHash.trim()} style={{ width: '100%', padding: '14px', borderRadius: 12, background: (loading || !txHash.trim()) ? 'rgba(0,206,201,0.2)' : 'linear-gradient(to right, #00B894, #00CEC9)', color: 'white', fontWeight: 700, fontSize: 15, border: 'none', cursor: (loading || !txHash.trim()) ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: (!loading && txHash.trim()) ? '0 8px 24px rgba(0,184,148,0.3)' : 'none' }}>
          {loading ? (
