...
export default function AdminMainDashboardPage() {
  ...
  const loadDashboard = async () => {
    try {
      ...
      setUsers(allUsers || []);
      const { count: activeCount } = await supabase.from('beneficiary_activations').select('*', { count: 'exact', head: true }).eq('payment_status', 'verified');
      ...
      setStats({
        total_beneficiaries: activeCount || 0, active_beneficiaries: activeCount || 0,
        ...
        <main style={{ maxWidth: 1200, margin: '0 auto', padding: '28px 24px 60px', position: 'relative', zIndex: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 24 }}>
            {[
              ...
              <div key={s.label} style={{ padding: '20px', borderRadius: 18, border: '1px solid rgba(0,206,201,0.12)', backgroundColor: 'rgba(255,255,255,0.03)', position: 'relative', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', top: 16, right: 16, width: 40, height: 40, borderRadius: 11, backgroundColor: s.bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{s.icon}</div>
          ...
          <div style={{ padding: '24px', borderRadius: 20, border: '1px solid rgba(0,206,201,0.15)', backgroundColor: 'rgba(255,255,255,0.02)', marginBottom: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
              <div>
                <p style={{ fontSize: 16, fontWeight: 700, color: 'white', margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ width: 4, height: 18, backgroundColor: '#00CEC9', borderRadius: 2, display: 'inline-block' }} />Users Management
                </p>
                  ...
                  {filteredUsers.slice(0, 20).map((u) => (
                    <tr key={u.id} style={{ borderBottom: '1px solid rgba(0,206,201,0.06)' }}>
                      ...
                      <td style={{ padding: '14px 16px 14px 0' }}>
                        <span style={{ padding: '3px 10px', borderRadius: 999, fontSize: 11, fontWeight: 600, backgroundColor: u.role === 'admin' ? 'rgba(155,89,182,0.2)' : u.role === 'philanthropist' ? 'rgba(0,184,148,0.15)' : 'rgba(0,206,201,0.15)', color: u.role === 'admin' ? '#9B59B6' : u.role === 'philanthropist' ? '#00B894' : '#00CEC9', border: '1px solid ' + (u.role === 'admin' ? 'rgba(155,89,182,0.4)' : u.role === 'philanthropist' ? 'rgba(0,184,148,0.3)' : 'rgba(0,206,201,0.3)'), textTransform: 'capitalize' }}>{u.role}</span>
                      </td>
