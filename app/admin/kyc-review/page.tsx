...
export default function AdminKYCReviewPage() {
  ...
  const loadKYCSubmissions = async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.from('kyc_submissions').select(`*, users:user_id (full_name, email, country, phone)`).eq('status', 'pending').order('submitted_at', { ascending: true });
  ...
  const loadTodayStats = async () => {
    ...
    const { count: approved } = await supabase.from('kyc_submissions').select('*', { count: 'exact', head: true }).eq('status', 'approved').gte('reviewed_at', today);
    const { count: rejected } = await supabase.from('kyc_submissions').select('*', { count: 'exact', head: true }).eq('status', 'rejected').gte('reviewed_at', today);
  ...
  const handleApprove = async () => {
    ...
    setSubmitting(true);
    try {
      const { error } = await supabase.from('kyc_submissions').update({ status: 'approved', reviewed_at: new Date().toISOString(), reviewed_by: user.id, updated_at: new Date().toISOString() }).eq('id', selectedKYC.id);
      if (error) throw error;

      if (selectedKYC.user_id) {
        const { data: targetUser } = await supabase.from('users').select('role').eq('id', selectedKYC.user_id).single();
        if (targetUser?.role !== 'admin') {
          await supabase.from('users').update({ role: 'philanthropist', updated_at: new Date().toISOString() }).eq('id', selectedKYC.user_id);
        }
        const { data: existingRecord } = await supabase.from('philanthropists').select('id').eq('user_id', selectedKYC.user_id).maybeSingle();
        if (!existingRecord) {
          const { error: insertError } = await supabase.from('philanthropists').insert({
            id: selectedKYC.user_id, user_id: selectedKYC.user_id, kyc_status: 'approved',
            kyc_submitted_at: selectedKYC.submitted_at || selectedKYC.created_at,
            kyc_approved_at: new Date().toISOString(), kyc_reviewer_id: user.id,
            kyc_review_notes: reviewNotes || null, act_balance: 1000, total_activated: 0,
          ...
          if (insertError) {
            ...
            await loadKYCSubmissions(); await loadTodayStats(); return;
          }
        } else {
          await supabase.from('philanthropists').update({ kyc_status: 'approved', kyc_approved_at: new Date().toISOString(), kyc_reviewer_id: user.id, kyc_review_notes: reviewNotes || null, updated_at: new Date().toISOString() }).eq('user_id', selectedKYC.user_id);
        }
      }
  ...
  const handleReject = async () => {
    ...
    setSubmitting(true);
    try {
      const { error } = await supabase.from('kyc_submissions').update({ status: 'rejected', rejection_reason: rejectionReason, reviewed_at: new Date().toISOString(), reviewed_by: user.id, updated_at: new Date().toISOString() }).eq('id', selectedKYC.id);
      if (error) throw error;
  ...
  if (selectedKYC) {
    ...
    return (
      <ProtectedRoute>
        <div style={pageStyle}>
          <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', overflow: 'hidden' }}><div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 600, height: 600, background: 'radial-gradient(circle, rgba(0,206,201,0.06) 0%, transparent 70%)', borderRadius: '50%' }} /></div>
          {toast && <div style={{ position: 'fixed', top: 20, right: 20, zIndex: 999, padding: '12px 20px', borderRadius: 12, backgroundColor: toast.type === 'success' ? 'rgba(0,184,148,0.15)' : 'rgba(255,107,107,0.15)', border: `1px solid ${toast.type === 'success' ? 'rgba(0,184,148,0.4)' : 'rgba(255,107,107,0.4)'}`, color: toast.type === 'success' ? '#00B894' : '#ff6b6b', fontWeight: 600, fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}>{toast.type === 'success' ? <CheckCircle style={{ width: 16, height: 16 }} /> : <XCircle style={{ width: 16, height: 16 }} />}{toast.msg}</div>}
          <header style={{ position: 'sticky', top: 0, zIndex: 50, borderBottom: '1px solid rgba(255,255,255,0.08)', backgroundColor: 'rgba(10,22,40,0.95)', backdropFilter: 'blur(12px)' }}>
            <div style={{ maxWidth: 900, margin: '0 auto', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
              <button onClick={() => setSelectedKYC(null)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.1)', backgroundColor: 'transparent', color: '#8FA3BF', cursor: 'pointer', fontSize: 13 }}><ArrowLeft style={{ width: 15, height: 15 }} /> Back</button>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Shield style={{ width: 18, height: 18, color: '#9B59B6' }} /><span style={{ fontWeight: 700, fontSize: 15 }}>KYC Review — {applicantName}</span></div>
            </div>
          </header>
          <main style={{ maxWidth: 900, margin: '0 auto', padding: '28px 20px 60px', position: 'relative', zIndex: 10 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20 }}>
              <div style={{ padding: 24, borderRadius: 18, border: '1px solid rgba(0,206,201,0.2)', backgroundColor: 'rgba(255,255,255,0.04)' }}>
                <h2 style={{ fontSize: 15, fontWeight: 700, color: 'white', marginBottom: 18, display: 'flex', alignItems: 'center', gap: 8 }}><User style={{ width: 16, height: 16, color: '#00CEC9' }} /> Applicant Information</h2>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {[
                    ...
                    <div key={f.label} style={{ display: 'flex', gap: 12 }}>
                      ...
                      <div><p style={{ fontSize: 11, color: '#8FA3BF', marginBottom: 2 }}>{f.label}</p><p style={{ fontSize: 14, color: 'white', fontWeight: 500, textTransform: f.label === 'ID Type' ? 'capitalize' : 'none' }}>{f.value || '—'}</p></div>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {selectedKYC.government_id_url && <div style={{ padding: 20, borderRadius: 18, border: '1px solid rgba(0,206,201,0.2)', backgroundColor: 'rgba(255,255,255,0.04)' }}><h3 style={{ fontSize: 13, fontWeight: 700, color: '#8FA3BF', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 6 }}><FileText style={{ width: 14, height: 14, color: '#00CEC9' }} /> Government ID</h3><img src={selectedKYC.government_id_url} alt="ID Document" style={{ width: '100%', borderRadius: 10, border: '1px solid rgba(0,206,201,0.2)', objectFit: 'cover' }} /></div>}
                {selectedKYC.face_capture_url && <div style={{ padding: 20, borderRadius: 18, border: '1px solid rgba(0,206,201,0.2)', backgroundColor: 'rgba(255,255,255,0.04)' }}><h3 style={{ fontSize: 13, fontWeight: 700, color: '#8FA3BF', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 6 }}><User style={{ width: 14, height: 14, color: '#00CEC9' }} /> Face Capture</h3><img src={selectedKYC.face_capture_url} alt="Face Capture" style={{ width: '100%', borderRadius: 10, border: '1px solid rgba(0,206,201,0.2)', objectFit: 'cover', maxHeight: 280 }} /></div>}
                {!selectedKYC.government_id_url && !selectedKYC.face_capture_url && <div style={{ padding: 24, borderRadius: 18, border: '1px solid rgba(255,193,7,0.2)', backgroundColor: 'rgba(255,193,7,0.05)', textAlign: 'center' }}><p style={{ fontSize: 13, color: '#ffc107' }}>No documents uploaded</p></div>}
              </div>
            </div>
            <div style={{ padding: 24, borderRadius: 18, border: '1px solid rgba(0,206,201,0.2)', backgroundColor: 'rgba(255,255,255,0.04)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <button onClick={() => setReviewAction(reviewAction === 'approve' ? null : 'approve')} style={{ padding: '12px', borderRadius: 12, border: `2px solid ${reviewAction === 'approve' ? 'rgba(0,184,148,0.6)' : 'rgba(0,184,148,0.2)'}`, backgroundColor: reviewAction === 'approve' ? 'rgba(0,184,148,0.15)' : 'transparent', color: '#00B894', fontWeight: 700, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><CheckCircle style={{ width: 18, height: 18 }} /> Approve</button>
                  <button onClick={() => setReviewAction(reviewAction === 'reject' ? null : 'reject')} style={{ padding: '12px', borderRadius: 12, border: `2px solid ${reviewAction === 'reject' ? 'rgba(255,107,107,0.6)' : 'rgba(255,107,107,0.2)'}`, backgroundColor: reviewAction === 'reject' ? 'rgba(255,107,107,0.15)' : 'transparent', color: '#ff6b6b', fontWeight: 700, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}><XCircle style={{ width: 18, height: 18 }} /> Reject</button>
                </div>
                {reviewAction && <button onClick={reviewAction === 'approve' ? handleApprove : handleReject} disabled={submitting} style={{ width: '100%', padding: '14px', borderRadius: 12, background: reviewAction === 'approve' ? 'linear-gradient(to right, #00B894, #00CEC9)' : 'linear-gradient(to right, #ff6b6b, #ee5a24)', color: 'white', fontWeight: 700, fontSize: 15, border: 'none', cursor: submitting ? 'not-allowed' : 'pointer', opacity: submitting ? 0.7 : 1 }}>{submitting ? 'Processing...' : `Confirm ${reviewAction === 'approve' ? 'Approval ✓' : 'Rejection ✗'}`}</button>}
              </div>
            </div>
          </main>
        </div>
  ...
  return (
    <ProtectedRoute>
      <div style={pageStyle}>
        <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', overflow: 'hidden' }}><div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 600, height: 600, background: 'radial-gradient(circle, rgba(108,63,200,0.06) 0%, transparent 70%)', borderRadius: '50%' }} /></div>
        {toast && <div style={{ position: 'fixed', top: 20, right: 20, zIndex: 999, padding: '12px 20px', borderRadius: 12, backgroundColor: toast.type === 'success' ? 'rgba(0,184,148,0.15)' : 'rgba(255,107,107,0.15)', border: `1px solid ${toast.type === 'success' ? 'rgba(0,184,148,0.4)' : 'rgba(255,107,107,0.4)'}`, color: toast.type === 'success' ? '#00B894' : '#ff6b6b', fontWeight: 600, fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}>{toast.type === 'success' ? <CheckCircle style={{ width: 16, height: 16 }} /> : <XCircle style={{ width: 16, height: 16 }} />}{toast.msg}</div>}
        <header style={{ position: 'sticky', top: 0, zIndex: 50, borderBottom: '1px solid rgba(255,255,255,0.08)', backgroundColor: 'rgba(10,22,40,0.95)', backdropFilter: 'blur(12px)' }}>
          <div style={{ maxWidth: 900, margin: '0 auto', padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => router.push('/admin')} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 14px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.1)', backgroundColor: 'transparent', color: '#8FA3BF', cursor: 'pointer', fontSize: 12 }}><ArrowLeft style={{ width: 13, height: 13 }} /> Admin Home</button>
          </div>
        </header>
        <main style={{ maxWidth: 900, margin: '0 auto', padding: '28px 20px 60px', position: 'relative', zIndex: 10 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 24 }}>
            {[
              ...
              <div key={stat.label} style={{ padding: '18px 16px', borderRadius: 16, border: `1px solid ${stat.border}`, backgroundColor: stat.bg, display: 'flex', alignItems: 'center', gap: 14 }}>
                ...
                <div><p style={{ fontSize: 26, fontWeight: 800, color: stat.color, lineHeight: 1 }}>{stat.value}</p><p style={{ fontSize: 12, color: '#8FA3BF', marginTop: 3 }}>{stat.label}</p></div>
              </div>
            ))}
          </div>
                ...
                {kycSubmissions.map((kyc, i) => {
                  ...
                  return (
                    <div key={kyc.id} style={{ padding: '16px 20px', borderBottom: i < kycSubmissions.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none', display: 'flex', alignItems: 'center', gap: 16 }}>
                      <div style={{ width: 44, height: 44, borderRadius: '50%', backgroundColor: 'rgba(0,206,201,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: '2px solid rgba(0,206,201,0.2)' }}><span style={{ fontSize: 16, fontWeight: 800, color: '#00CEC9' }}>{name[0].toUpperCase()}</span></div>
                      <div style={{ flex: 1 }}>
                      ...
                      <button onClick={() => { setSelectedKYC(kyc); setReviewAction(null); setReviewNotes(''); setRejectionReason(''); }} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: 10, background: 'linear-gradient(to right, #00CEC9, #00B894)', color: 'white', fontWeight: 700, fontSize: 13, border: 'none', cursor: 'pointer', flexShrink: 0 }}><Eye style={{ width: 14, height: 14 }} /> Review</button>
                    </div>
                  );
                })}
              </div>
