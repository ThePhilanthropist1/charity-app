...
async function getPhilRecord(userId: string) {
  ...
  const { data: byId } = await supabase.from('philanthropists').select('*').eq('id', userId).maybeSingle();
  return byId || null;
}
...
export default function PhilanthropistDashboardPage() {
  ...
  useEffect(() => {
    ...
    realtimeChannel.current = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'philanthropists', filter: `user_id=eq.${user.id}` }, (payload: any) => {
        setActBalance(payload.new.act_balance ?? ACT_INITIAL_BALANCE);
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'beneficiary_activations' }, () => { refreshCounts(); })
      .subscribe();

    return () => { if (realtimeChannel.current) supabase.removeChannel(realtimeChannel.current); };
  }, [user, authLoading]);
  ...
  const refreshCounts = async () => {
    ...
    try {
      const [{ count: activated }, { count: rejected }, { data: pending }] = await Promise.all([
        supabase.from('beneficiary_activations').select('id', { count: 'exact', head: true }).eq('payment_status', 'verified').eq('philanthropist_id', user.id),
        supabase.from('beneficiary_activations').select('id', { count: 'exact', head: true }).eq('payment_status', 'rejected').eq('philanthropist_id', user.id),
        supabase.from('beneficiary_activations').select(`*, users:user_id (id, full_name, email, country, profile_picture)`).eq('payment_status', 'pending').order('created_at', { ascending: true }),
      ]);
  ...
  const handleActivateFromQueue = async (item: any) => {
    ...
    if (actBalance < ACT_PER_ACTIVATION) { showToast(`Insufficient ACT balance. Need ${ACT_PER_ACTIVATION} ACT to activate.`, 'error'); setShowRefill(true); return; }
      ...
      <div style={{ minHeight: '100vh', backgroundColor: '#0A1628', color: 'white', fontFamily: 'sans-serif' }}>
        ...
        {toast && (
          <div style={{ position: 'fixed', top: 20, right: 20, zIndex: 1000, padding: '14px 20px', borderRadius: 14, backgroundColor: toastBg, border: `2px solid ${toastBorder}`, color: toastColor, fontWeight: 600, fontSize: 14, display: 'flex', alignItems: 'flex-start', gap: 10, boxShadow: '0 12px 40px rgba(0,0,0,0.5)', maxWidth: 420 }}>
            ...
            <button onClick={() => setToast(null)} style={{ background: 'none', border: 'none', color: 'inherit', cursor: 'pointer', padding: 0, flexShrink: 0, opacity: 0.6 }}><X style={{ width: 14, height: 14 }} /></button>
          </div>
        )}
        ...
        <main style={{ maxWidth: 860, margin: '0 auto', padding: '28px 20px 80px', position: 'relative', zIndex: 10 }}>
          <div style={{ marginBottom: 28 }}>
            ...
            <h1 style={{ fontSize: 'clamp(22px, 4vw, 30px)', fontWeight: 800, marginBottom: 6 }}>Welcome, {user?.full_name?.split(' ')[0] || 'Philanthropist'} 👐</h1>
          ...
          <div style={{ marginBottom: 20, padding: '24px', borderRadius: 20, background: 'linear-gradient(135deg, #0d2137 0%, #0a1628 60%, #0d2137 100%)', border: `2px solid ${actColor}40`, position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 18 }}>
              <div>
                ...
                <p style={{ fontSize: 12, color: '#8FA3BF', marginTop: 6 }}><strong style={{ color: 'white' }}>{Math.floor(actBalance / ACT_PER_ACTIVATION)}</strong> activations remaining · <strong style={{ color: '#67e8f9' }}>{ACT_PER_ACTIVATION} ACT</strong> per activation</p>
              </div>
              <button onClick={() => setShowRefill(true)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '11px 20px', borderRadius: 12, background: `linear-gradient(to right, ${actColor}, #00CEC9)`, color: 'white', fontWeight: 700, fontSize: 13, border: 'none', cursor: 'pointer', flexShrink: 0 }}><RefreshCw style={{ width: 15, height: 15 }} /> Refill ACT</button>
            </div>
            ...
            {actBalance < 100 && (
              <div style={{ marginTop: 14, padding: '10px 14px', borderRadius: 10, backgroundColor: 'rgba(255,107,107,0.1)', border: '1px solid rgba(255,107,107,0.25)', display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <p style={{ fontSize: 12, color: '#ff6b6b', margin: 0 }}>Low balance! <button onClick={() => setShowRefill(true)} style={{ background: 'none', border: 'none', color: '#ff6b6b', textDecoration: 'underline', cursor: 'pointer', fontWeight: 700, padding: 0, fontSize: 12 }}>Refill now →</button></p>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14, marginBottom: 24 }}>
            {[
              { label: 'My Total Activated', value: totalActivated, color: '#00B894', bg: 'rgba(0,184,148,0.08)', border: 'rgba(0,184,148,0.2)', icon: <UserCheck style={{ width: 18, height: 18, color: '#00B894' }} /> },
              { label: 'My Total Rejected', value: totalRejected, color: '#ff6b6b', bg: 'rgba(255,107,107,0.08)', border: 'rgba(255,107,107,0.2)', icon: <XCircle style={{ width: 18, height: 18, color: '#ff6b6b' }} /> },
          ...
          <div style={{ padding: '16px 20px', borderRadius: 14, border: '1px solid rgba(255,193,7,0.2)', backgroundColor: 'rgba(255,193,7,0.05)', marginBottom: 24 }}>
            <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 12, color: '#8FA3BF', lineHeight: 1.7, margin: 0 }}>Only charge beneficiaries <strong style={{ color: 'white' }}>$1 USD equivalent</strong> in local fiat. Each activation costs <strong style={{ color: '#67e8f9' }}>{ACT_PER_ACTIVATION} ACT</strong>. Refill with <strong style={{ color: 'white' }}>${REFILL_COST_USD} USDT</strong> for <strong style={{ color: '#00B894' }}>{REFILL_ACT_AMOUNT} ACT</strong>.<strong style={{ color: '#ff6b6b' }}> Overcharging = immediate removal.</strong></p>
              </div>
            </div>
          </div>
          ...
          <ActivateByEmail
            ...
            onSuccess={(newBal, name) => { setActBalance(newBal); showToast(`✅ ${name} activated! −${ACT_PER_ACTIVATION} ACT deducted. Balance: ${newBal} ACT`, 'success'); loadAll(); }}
          ...
          <div style={{ borderRadius: 18, border: '1px solid rgba(0,206,201,0.15)', backgroundColor: 'rgba(255,255,255,0.03)', overflow: 'hidden', marginTop: 20 }}>
            <div style={{ padding: '18px 20px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              {pendingUsers.length > 0 && <span style={{ marginLeft: 'auto', fontSize: 12, padding: '3px 10px', borderRadius: 999, backgroundColor: 'rgba(255,193,7,0.15)', color: '#ffc107', fontWeight: 700, border: '1px solid rgba(255,193,7,0.3)' }}>{pendingUsers.length} pending</span>}
            </div>
                ...
                {pendingUsers.map((item, i) => {
                  ...
                  return (
                    <div key={item.id} style={{ padding: '16px 20px', borderBottom: i < pendingUsers.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none', display: 'flex', alignItems: 'center', gap: 14 }}>
                      {u?.profile_picture ? <img src={u.profile_picture} alt={name} style={{ width: 44, height: 44, borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(0,206,201,0.25)', flexShrink: 0 }} /> : <div style={{ width: 44, height: 44, borderRadius: '50%', backgroundColor: 'rgba(0,206,201,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, border: '2px solid rgba(0,206,201,0.2)' }}><span style={{ fontSize: 16, fontWeight: 800, color: '#00CEC9' }}>{(name[0] || 'U').toUpperCase()}</span></div>}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                        <button onClick={() => handleRejectFromQueue(item)} disabled={isBusy} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '9px 14px', borderRadius: 10, background: 'rgba(255,107,107,0.1)', color: '#ff6b6b', fontWeight: 700, fontSize: 13, border: '1px solid rgba(255,107,107,0.25)', cursor: isBusy ? 'not-allowed' : 'pointer', opacity: isBusy ? 0.5 : 1 }}>
                          {isRejecting ? <div style={{ width: 13, height: 13, border: '2px solid rgba(255,107,107,0.3)', borderTop: '2px solid #ff6b6b', borderRadius: '50%', animation: 'spin 1s linear infinite' }} /> : <XCircle style={{ width: 13, height: 13 }} />}
                          {isRejecting ? '...' : 'Reject'}
                        </button>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                          <button onClick={() => handleActivateFromQueue(item)} disabled={isBusy || !canActivate} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 16px', borderRadius: 10, background: !canActivate ? 'rgba(255,107,107,0.15)' : isActivating ? 'rgba(0,184,148,0.3)' : 'linear-gradient(to right, #00B894, #00CEC9)', color: !canActivate ? '#ff6b6b' : 'white', fontWeight: 700, fontSize: 13, border: !canActivate ? '1px solid rgba(255,107,107,0.3)' : 'none', cursor: (isBusy || !canActivate) ? 'not-allowed' : 'pointer' }}>
                            {isActivating ? <div style={{ width: 13, height: 13, border: '2px solid rgba(255,255,255,0.3)', borderTop: '2px solid white', borderRadius: '50%', animation: 'spin 1s linear infinite' }} /> : <CheckCircle style={{ width: 13, height: 13 }} />}
...
function ActivateByEmail({ userId, actBalance, onSuccess, onAlreadyActive, onError, onLowBalance }: {
  userId: string; actBalance: number;
  ...
  const handleActivate = async () => {
    ...
    if (foundUser.activation?.payment_status === 'verified') { onAlreadyActive(name); return; }
    if (actBalance < ACT_PER_ACTIVATION) { onError(`Insufficient ACT balance. Need ${ACT_PER_ACTIVATION} ACT.`); onLowBalance(); return; }
    setActivating(true);
    try {
  ...
  return (
    <div style={{ padding: '22px', borderRadius: 18, border: '1px solid rgba(0,206,201,0.2)', backgroundColor: 'rgba(255,255,255,0.03)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
        <div style={{ width: 36, height: 36, borderRadius: 10, backgroundColor: 'rgba(0,206,201,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Mail style={{ width: 16, height: 16, color: '#00CEC9' }} /></div>
        <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: foundUser || notFound ? 14 : 0 }}>
        <input placeholder="Enter beneficiary's registered email address" value={email} onChange={(e) => { setEmail(e.target.value); setFoundUser(null); setNotFound(false); }} onKeyDown={(e) => e.key === 'Enter' && searchUser()} style={{ flex: 1, padding: '12px 14px', borderRadius: 10, backgroundColor: '#0A1628', border: '1px solid rgba(0,206,201,0.2)', color: 'white', fontSize: 13, outline: 'none' }} />
        <button onClick={searchUser} disabled={searching || !email.trim()} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '12px 20px', borderRadius: 10, background: 'linear-gradient(to right, #00CEC9, #00B894)', color: 'white', fontWeight: 700, fontSize: 13, border: 'none', cursor: (searching || !email.trim()) ? 'not-allowed' : 'pointer', opacity: (searching || !email.trim()) ? 0.6 : 1, whiteSpace: 'nowrap' }}>
          <Search style={{ width: 14, height: 14 }} />{searching ? 'Searching...' : 'Search'}
        </button>
      </div>
      {notFound && <div style={{ padding: '12px 14px', borderRadius: 10, backgroundColor: 'rgba(255,107,107,0.08)', border: '1px solid rgba(255,107,107,0.2)' }}><p style={{ fontSize: 13, color: '#ff6b6b', margin: 0 }}>No account found. The user must register first.</p></div>}
      {foundUser && (
        <div style={{ padding: '16px', borderRadius: 12, backgroundColor: 'rgba(0,206,201,0.04)', border: '1px solid rgba(0,206,201,0.15)', display: 'flex', alignItems: 'center', gap: 14 }}>
          {foundUser.profile_picture ? <img src={foundUser.profile_picture} alt={foundUser.full_name} style={{ width: 50, height: 50, borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(0,206,201,0.3)', flexShrink: 0 }} /> : <div style={{ width: 50, height: 50, borderRadius: '50%', backgroundColor: 'rgba(0,206,201,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><span style={{ fontSize: 20, fontWeight: 800, color: '#00CEC9' }}>{(foundUser.full_name || foundUser.email || 'U')[0].toUpperCase()}</span></div>}
          <div style={{ flex: 1 }}>
            ...
            {foundUser.activation?.payment_status === 'verified' ? <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 999, backgroundColor: 'rgba(0,184,148,0.15)', color: '#00B894', border: '1px solid rgba(0,184,148,0.3)', fontWeight: 600 }}>✓ Already Active</span> : <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 999, backgroundColor: 'rgba(255,193,7,0.15)', color: '#ffc107', border: '1px solid rgba(255,193,7,0.3)', fontWeight: 600 }}>Not Activated</span>}
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, flexShrink: 0 }}>
            {foundUser.activation?.payment_status === 'verified' ? (
              <button onClick={() => onAlreadyActive(foundUser.full_name || foundUser.email)} style={{ padding: '10px 16px', borderRadius: 10, backgroundColor: 'rgba(0,184,148,0.1)', border: '1px solid rgba(0,184,148,0.3)', color: '#00B894', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>✓ Active</button>
            ) : (
              <>
...
function RefillModal({ onClose, onSuccess, userId, currentBalance }: {
  ...
  const copyWallet = () => { navigator.clipboard.writeText(WALLET_ADDRESS); setCopied(true); setTimeout(() => setCopied(false), 2000); };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { error: dbErr } = await supabase.from('philanthropists').update({ act_balance: newBalance, last_refill_hash: cleanHash, last_refill_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', record.id);
      if (dbErr) throw dbErr;
      onSuccess(newBalance);
    } catch (e: any) { setError('Verification failed: ' + (e.message || 'Please try again.')); }
    finally { setLoading(false); }
  };
      ...
      <div style={{ width: '100%', maxWidth: 520, backgroundColor: '#0F1F35', border: '1px solid rgba(0,206,201,0.2)', borderRadius: 22, padding: 28, boxShadow: '0 40px 80px rgba(0,0,0,0.7)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 42, height: 42, borderRadius: 12, background: 'linear-gradient(to right, #00B894, #00CEC9)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><RefreshCw style={{ width: 18, height: 18, color: 'white' }} /></div>
            <div>
          ...
          <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: '50%', backgroundColor: 'rgba(255,255,255,0.06)', border: 'none', color: '#8FA3BF', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X style={{ width: 16, height: 16 }} /></button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 20 }}>
          <div style={{ padding: '14px 16px', borderRadius: 12, backgroundColor: 'rgba(0,206,201,0.04)', border: '1px solid rgba(0,206,201,0.12)' }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: '#00CEC9', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ width: 20, height: 20, borderRadius: '50%', background: 'linear-gradient(to right, #00CEC9, #00B894)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: 'white', flexShrink: 0 }}>1</span>Send exactly ${REFILL_COST_USD} USDT (BEP20) to:</p>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', backgroundColor: '#0A1628', borderRadius: 8, border: '1px solid rgba(0,206,201,0.15)' }}>
              ...
              <button onClick={copyWallet} style={{ background: 'none', border: 'none', cursor: 'pointer', color: copied ? '#00B894' : '#67e8f9', flexShrink: 0, padding: 0 }}>{copied ? <CheckCircle style={{ width: 16, height: 16 }} /> : <Copy style={{ width: 16, height: 16 }} />}</button>
            </div>
          ...
          <div style={{ padding: '14px 16px', borderRadius: 12, backgroundColor: 'rgba(0,184,148,0.04)', border: '1px solid rgba(0,184,148,0.12)' }}>
            <p style={{ fontSize: 12, fontWeight: 700, color: '#00B894', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ width: 20, height: 20, borderRadius: '50%', background: 'linear-gradient(to right, #00B894, #00CEC9)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: 'white', flexShrink: 0 }}>2</span>Paste your BSC transaction hash:</p>
        ...
        <div style={{ padding: '12px 16px', borderRadius: 10, backgroundColor: 'rgba(0,184,148,0.06)', border: '1px solid rgba(0,184,148,0.15)', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <p style={{ fontSize: 12, color: '#8FA3BF', margin: 0 }}>You will receive <strong style={{ color: '#00B894' }}>{REFILL_ACT_AMOUNT} ACT</strong> — activates <strong style={{ color: 'white' }}>{REFILL_ACT_AMOUNT / ACT_PER_ACTIVATION} beneficiaries</strong>. New balance: <strong style={{ color: '#67e8f9' }}>{currentBalance + REFILL_ACT_AMOUNT} ACT</strong>.</p>
        </div>
